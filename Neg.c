...
*  @Author:             JSE
*  @Date:                 2024-5-16
*  @Description:   Neg算子的性能测试代码，包含普通C-for循环、rvv-intrisic函数、rvv-内联汇编3种算子实现方法； 
                                    Neg算子执行元素逐个取负操作
                                    输入输出数据类型为int32，维度为1；
                                    两种rvv寄存器分组模式都为m4
************************************************************************************************************/
...
#define N 2555            //N为输入数据尺寸，可修改
int input[N];

/******************************************************************
*  Function:            Neg_c
*  Description:       Neg算子的C-for循环实现方法，元素逐个取负
*  Parameter:         
*  @inputA              输入数据
*  @inputB              输入数据
*  @output              输出数据
...
*  Return:                 None
******************************************************************/
void Neg_c(int size, int *input, int *output) {
...
/******************************************************************
*  Function:            Neg_intr
*  Description:       Neg算子的rvv-intrisic函数实现方法
*  Parameter:         同Neg_c
*  Return:                 None
******************************************************************/
void Neg_intr(int size, int *input, int *output_vector) {
    vint32m4_t vinput;
	vint32m4_t voutput;
	unsigned int gvl = 0;
    gvl = vsetvlmax_e32m4();
	for (int i = 0; i < size;) {
		...
		voutput = vneg_v_i32m4(vinput,  gvl);
...
/**********************************************************************
*  Function:            Neg_assembly
*  Description:       Neg算子的rvv-内联汇编实现方法，vneg.v是RVV1.0的指令
*  Parameter:         同Neg_c
*  Return:                 None
**********************************************************************/
/* void Neg_assembly(int size, int *input, int *output_assembly) {
    asm volatile(
        "1:\n\t"
        "vsetvli        t0, %2, e32, m4\n\t"
        "vle.v       v8, (%0)\n\t"
...
int main() {
    ...
    Neg_c(N, input, output);
    clock_t end_time = clock();
    double cpu_time_used = ((double) (end_time - start_time)) / CLOCKS_PER_SEC;
    printf("Output after applying Neg_c:\n");
    for (int i = 0; i < 16; ++i) {
        printf("%d ", output[i]);
    }
    printf("\n");

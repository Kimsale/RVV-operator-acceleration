...
*  @Author:             JSE
*  @Date:                 2024-5-16
*  @Description:   Or算子的性能测试代码，包含普通C-for循环、rvv-intrisic函数、rvv-内联汇编3种算子实现方法； 
                                    Or算子执行逻辑或操作
                                    输入输出数据类型为int32，维度为1；
                                    两种rvv寄存器分组模式都为m4
************************************************************************************************************/
...
*  Return:                 None
******************************************************************/
void Or_c(int size, int *inputA, int *inputB, int *output) {
...
/******************************************************************
*  Function:            Or_intr
*  Description:       Or算子的rvv-intrisic函数实现方法
*  Parameter:         同Or_c
*  Return:                 None
******************************************************************/
void Or_intr(int size, int *inputA, int *inputB, int *outputTensor_vector) {
    vint32m4_t vinputA;
    vint32m4_t vinputB;
	vint32m4_t voutput;
	unsigned int gvl = 0;
    gvl = vsetvlmax_e32m4();
	for (int i = 0; i < size;) {
		vinputA = vle32_v_i32m4(&inputA[i], gvl);
        vinputB = vle32_v_i32m4(&inputB[i], gvl);
		voutput = vor_vv_i32m4(vinputA, vinputB,  gvl);
        vse32_v_i32m4(&outputTensor_vector[i], voutput, gvl);
		i += gvl;
	}
}

/******************************************************************
*  Function:            Or_assembly
*  Description:       Or算子的rvv-内联汇编实现方法
*  Parameter:         同Or_c
*  Return:                 None
******************************************************************/
void Or_assembly(int size, int *inputA, int *inputB,
...
int main() {
    ...
    Or_c(N, inputA, inputB, output);
    clock_t end_time = clock();
    double cpu_time_used = ((double) (end_time - start_time)) / CLOCKS_PER_SEC;
    printf("Output after applying Or_c:\n");
    for (int i = 0; i < N; ++i) {
        printf("%d ", output[i]);
    }
    printf("\n");
